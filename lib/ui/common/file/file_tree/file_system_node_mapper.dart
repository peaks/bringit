...
class FileSystemNodeMapper {
  ...
  final Map<String, GitFileStatus> _status = <String, GitFileStatus>{};

  final IconFetcher _iconFetcher;

  set status(List<GitFileStatus> files) {
    _status.clear();
    for (final GitFileStatus element in files) {
  ...
  Map<String, GitFileStatus> _findParentUntrackedPath(String absolutePath) {
    final Map<String, GitFileStatus> untrackedPath =
        Map<String, GitFileStatus>.from(_status);

    untrackedPath.removeWhere((String key, GitFileStatus value) =>
