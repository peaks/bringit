...
void main() {
  ...
  List<StatusFile> doParseTest(String statusLine) =>
  ...
  void expectWillMatch(
    ...
    final List<StatusFile> actual = doParseTest(originalPattern);
  ...
  test('provided prefix is appended to file path', () {
    ...
    final List<StatusFile> result = interpreter.parse('?? $gitFilePath');
  ...
  test('separator is appended to prefix if missing', () {
    ...
    final List<StatusFile> result = interpreter.parse('?? test.txt');
    expect(result.first.fileAbsolutePath,
